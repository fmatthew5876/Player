...
namespace {
	...
	BitmapFontGlyph const* find_glyph(const T& glyphset, char32_t code) {
		auto iter = std::lower_bound(glyphset.begin(), glyphset.end(), code);
		if(iter != glyphset.end() && iter->code == code) {
			return &*iter;
		} else {
			return NULL;
		}
...
Font::GlyphRet BitmapFont::Glyph(char32_t code) {
	// FIXME: When Player #1960 is merged we can remove this lazy init.
