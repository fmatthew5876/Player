		...
		if (tret.is_escape) {
			...
			default:
				if (ch == Player::escape_char) {
					DrawGlyph(*font, *system, Player::escape_char, instant_speed, false);
				}
				break;
			}
			continue;
...
void Window_Message::InputNumber() {
	if (Input::IsTriggered(Input::DECISION)) {
		Game_System::SePlay(Game_System::GetSystemSE(Game_System::SFX_Decision));
		Game_Variables.Set(pending_message.GetNumberInputVariable(), number_input_window->GetNumber());
